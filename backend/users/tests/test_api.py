...
class TestUserGetRolesApi(TestCase):
    ...
    def test_get_roles_success(self):
        response = self.client.get(reverse("api:users:get-user-roles", kwargs={"user_id": self.user.pk}))
    ...
    def test_get_roles_unauthenticated_redirect_to_login(self):
        ...
        api_url = reverse("api:users:get-user-roles", kwargs={"user_id": self.user.pk})
